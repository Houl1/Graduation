...
# 搭建模型
x = keras.layers.Input(shape=(img_dim, img_dim, 1))
h = x

for i in range(2):
    filters *= 2
    h = keras.layers.Conv2D(filters=filters,
               kernel_size=3,
               strides=2,
               padding='same')(h)
...
z_mean = keras.layers.Dense(latent_dim)(h) # p(z|x)的均值
z_log_var = keras.layers.Dense(latent_dim)(h) # p(z|x)的方差

encoder = keras.models.Model(x, z_mean) # 通常认为z_mean就是所需的隐变量编码


z = keras.layers.Input(shape=(latent_dim,))
h = z
h = keras.layers.Dense(np.prod(h_shape))(h)
h = keras.layers.Reshape(h_shape)(h)

for i in range(2):
    h = keras.layers.Conv2DTranspose(filters=filters,
                        kernel_size=3,
                        strides=1,
                        padding='same')(h)
    h = keras.layers.LeakyReLU(0.2)(h)
...
x_recon = keras.layers.Conv2DTranspose(filters=1,
                          kernel_size=3,
                          activation='sigmoid',
                          padding='same')(h)


decoder = keras.models.Model(z, x_recon) # 解码器
generator = decoder


z = keras.layers.Input(shape=(latent_dim,))
y = keras.layers.Dense(intermediate_dim, activation='relu')(z)
y = keras.layers.Dense(num_classes, activation='softmax')(y)

classfier = keras.models.Model(z, y) # 隐变量分类器
...
class Gaussian(keras.models.Layer):
...
# 建立模型
vae = keras.models.Model(x, [x_recon, z_prior_mean, y])
